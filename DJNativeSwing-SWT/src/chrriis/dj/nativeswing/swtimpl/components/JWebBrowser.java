...
public class JWebBrowser extends NSPanelComponent {
	...
	private static final NSOption XUL_RUNNER_RUNTIME_OPTION = new NSOption(
			USE_XULRUNNER_RUNTIME_OPTION_KEY);
	...
	private static final NSOption WEBKIT_RUNTIME_OPTION = new NSOption(
			USE_WEBKIT_RUNTIME_OPTION_KEY);
	...
	/**
	 * Create a decorator for this web browser. This method can be overridden so
	...
	protected WebBrowserDecorator createWebBrowserDecorator(
		...
		if (webBrowserDecoratorFactory != null) {
			WebBrowserDecorator webBrowserDecorator = webBrowserDecoratorFactory
					.createWebBrowserDecorator(this, renderingComponent);
	...
	private static INativeWebBrowserStatic webBrowserStatic = NativeCoreObjectFactory
			.create(INativeWebBrowserStatic.class,
	...
	public static void copyAppearance(JWebBrowser fromWebBrowser,
		...
		toWebBrowser.setLocationBarVisible(fromWebBrowser
		...
		if (optionMap.get(USE_WEBKIT_RUNTIME_OPTION_KEY) != null) {
			if (runtime != INativeWebBrowser.WebBrowserRuntime.DEFAULT) {
				throw new IllegalStateException(
						"Only one web browser runtime can be specified!");
			}
			runtime = INativeWebBrowser.WebBrowserRuntime.WEBKIT;
		}
		nativeWebBrowser = NativeCoreObjectFactory
				.create(INativeWebBrowser.class,
						"chrriis.dj.nativeswing.swtimpl.components.core.NativeWebBrowser",
						new Class<?>[] { JWebBrowser.class,
								INativeWebBrowser.WebBrowserRuntime.class },
						new Object[] { this, runtime });
		initialize((NativeComponent) nativeWebBrowser);
		webBrowserDecorator = createWebBrowserDecorator(nativeWebBrowser
				.createEmbeddableComponent(optionMap));
	...
	public Object executeJavascriptWithResult(String javascript) {
		...
		// "  return null;" +
		// "}");
		Object[] result = executeJavascriptWithCommandResult(
				"[[getScriptResult]]", "try {" + "  " + COMMAND_FUNCTION
						+ "('[[getScriptResult]]', (function() {" + javascript
						+ "})());" + "} catch(exxxxx) {" + "  "
	...
	private static class NCommandListener extends WebBrowserAdapter {
		...
		private NCommandListener(String command,
		...
		public void commandReceived(WebBrowserCommandEvent e) {
			if (command.equals(e.getCommand())) {
				...
				((INativeWebBrowser) e.getWebBrowser().getNativeComponent())
						.removeWebBrowserListener(this);
			}
		}
	}

	private Object[] executeJavascriptWithCommandResult(final String command,
			String script) {
		if (!((NativeComponent) nativeWebBrowser).isNativePeerInitialized()) {
			return null;
		}
		final AtomicReference<Object[]> result = new AtomicReference<Object[]>();
		WebBrowserAdapter webBrowserListener = new NCommandListener(command,
				result);
	...
	public void addWebBrowserListener(WebBrowserListener listener) {
		...
		NativeWebBrowserListener nativeWebBrowserListener = new NativeWebBrowserListener(
				listener);
		webBrowserListenerToNativeWebBrowserListenerMap.put(listener,
				nativeWebBrowserListener);
	...
	public void setDefaultPopupMenuRegistered(
		...
		nativeWebBrowser
				.setDefaultPopupMenuRegistered(isDefaultPopupMenuRegistered);
	}
