 ...
 */
class NativeWebBrowser extends SWTNativeComponent implements
		INativeWebBrowser {
	...
	private static class CMJ_closeWindow extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			JWebBrowser webBrowser = nativeWebBrowser == null ? null
					: nativeWebBrowser.webBrowser.get();
			if (webBrowser == null) {
				return null;
			}
			Object[] listeners = nativeWebBrowser.listenerList
					.getListenerList();
	...
	private static class CMJ_createWindow extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			for (int i = listeners.length - 2; i >= 0 && jWebBrowser != null; i -= 2) {
				if (listeners[i] == WebBrowserListener.class) {
					if (e == null) {
						e = new WebBrowserWindowWillOpenEvent(webBrowser,
								jWebBrowser);
					}
			...
			if (!jWebBrowser.isNativePeerInitialized()) {
				Window windowAncestor = SwingUtilities
						.getWindowAncestor(jWebBrowser);
				if (windowAncestor == null) {
					Window parentWindow = e.isDialogWindow() ? SwingUtilities
							.getWindowAncestor(webBrowser) : null;
					windowAncestor = (Window) WebBrowserWindowFactory.create(
							parentWindow, jWebBrowser);
				}
				jWebBrowser.getNativeComponent().initializeNativePeer();
			}
			return ((NativeWebBrowser) jWebBrowser.getNativeComponent())
					.getComponentID();
		}
	}
	...
	private static class CMJ_showWindow extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			NativeWebBrowser nativeWebBrowser = (NativeWebBrowser) getNativeComponent();
			final JWebBrowser webBrowser = nativeWebBrowser == null ? null
					: nativeWebBrowser.webBrowser.get();
			if (webBrowser == null) {
				return null;
			}
			int componentID = (Integer) args[0];
			final JWebBrowser newWebBrowser = ((NativeWebBrowser) getNativeComponentRegistry()
					.get(componentID)).webBrowser.get();
			...
			JWebBrowserWindow browserWindow = newWebBrowser
					.getWebBrowserWindow();
			if (browserWindow != null) {
				if (size != null) {
					((Window) browserWindow).validate();
					Dimension windowSize = browserWindow.getSize();
					Dimension webBrowserSize = ((NativeWebBrowser) browserWindow
							.getWebBrowser().getNativeComponent()).embeddableComponent
							.getSize();
					if (size.width > 0) {
			...
			for (int i = listeners.length - 2; i >= 0 && newWebBrowser != null; i -= 2) {
				if (listeners[i] == WebBrowserListener.class) {
					if (e == null) {
						e = new WebBrowserWindowOpeningEvent(webBrowser,
								newWebBrowser, location, size);
					}
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							JWebBrowserWindow browserWindow = newWebBrowser
									.getWebBrowserWindow();
							if (browserWindow != null
									&& !newWebBrowser.getNativeComponent()
	...
	private static class CMJ_locationChanged extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			for (int i = listeners.length - 2; i >= 0; i -= 2) {
				if (listeners[i] == WebBrowserListener.class) {
					if (e == null) {
						e = new WebBrowserNavigationEvent(webBrowser, location,
								isTopFrame);
					}
	...
	private static class CMJ_commandReceived extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			for (int i = listeners.length - 2; i >= 0; i -= 2) {
				if (listeners[i] == WebBrowserListener.class) {
					if (e == null) {
						e = new WebBrowserCommandEvent(webBrowser, command,
								arguments);
					}
					WebBrowserListener webBrowserListener = (WebBrowserListener) listeners[i + 1];
					if (!isInternal
							|| webBrowserListener.getClass().getName()
									.startsWith("chrriis.")) {
	...
	private static class CMJ_locationChangeCanceled extends
		...
		public Object run(Object[] args) {
			...
			for (int i = listeners.length - 2; i >= 0; i -= 2) {
				if (listeners[i] == WebBrowserListener.class) {
					...
					((WebBrowserListener) listeners[i + 1])
							.locationChangeCanceled(e);
				}
			}
			return null;
		}
	...
	private static class CMJ_updateLoadingProgress extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
			NativeWebBrowser nativeWebBrowser = (NativeWebBrowser) getNativeComponent();
	...
	protected static Control createControl(Composite parent, Object[] parameters) {
		...
		if (xulRunnerPath != null) {
			NSSystemPropertySWT.ORG_ECLIPSE_SWT_BROWSER_XULRUNNERPATH
					.set(xulRunnerPath);
		} else {
			xulRunnerPath = NSSystemPropertySWT.ORG_ECLIPSE_SWT_BROWSER_XULRUNNERPATH
					.get();
			if (xulRunnerPath == null) {
		...
		browser.addOpenWindowListener(new OpenWindowListener() {
			public void open(WindowEvent e) {
				...
				e.required = true;
				final Integer componentID = (Integer) new CMJ_createWindow()
						.syncExec(browser);
				...
				if (componentID == null) {
					...
					isDisposed = false;
					newWebBrowser = (Browser) NativeComponent
							.getControlRegistry().get(componentID);
				}
				e.browser = newWebBrowser;
				newWebBrowser
						.addVisibilityWindowListener(new VisibilityWindowAdapter() {
							@Override
							public void show(WindowEvent e) {
								...
								if (isDisposed) {
									...
									Point location = e.location == null ? null
											: new Point(e.location.x,
													e.location.y);
									// For some reasons we get (1; 1) with
									// WebKit if the size is not set. Let's
									// consider it means null...
									Dimension size = e.size == null
											|| e.size.x == 1 && e.size.y == 1 ? null
											: new Dimension(e.size.x, e.size.y);
									new CMJ_showWindow().asyncExec(
											newWebBrowser, componentID,
											e.menuBar, e.toolBar, e.addressBar,
											e.statusBar, location, size);
								}
							}
						});
			}
		...
		browser.addLocationListener(new LocationListener() {
			...
			public void changing(LocationEvent e) {
				...
				if (location.startsWith(COMMAND_LOCATION_PREFIX)) {
					e.doit = false;
					String query = location.substring(COMMAND_LOCATION_PREFIX
							.length());
					if (query.endsWith("/")) {
					...
					String command = queryElementList.isEmpty() ? ""
							: queryElementList.remove(0);
				...
				e.doit = Boolean.TRUE.equals(new CMJ_locationChanging()
						.syncExec(browser, location, e.top));
				if (!e.doit) {
					browser.setData("Browser.loading", false);
					new CMJ_locationChangeCanceled().asyncExec(browser,
							location, e.top);
				}
			}
		});
		...
		browser.addStatusTextListener(new StatusTextListener() {
			public void changed(StatusTextEvent e) {
				final String oldStatus = (String) browser
						.getData("CMJ_updateStatus.status");
				final String newStatus = e.text;
				if (newStatus.startsWith(COMMAND_STATUS_PREFIX)) {
					// XULRunner on Linux: "window" is not defined when
					// synchronous... so we defer.
					e.display.asyncExec(new Runnable() {
						public void run() {
							browser.execute(fixJavascript(
									browser,
									"if(decodeURIComponent('"
											+ Utils.encodeURL(newStatus)
											+ "') == window.status) {window.status = decodeURIComponent('"
											+ Utils.encodeURL(oldStatus == null ? ""
													: oldStatus) + "');}"));
						}
					});
					String query = newStatus.substring(COMMAND_STATUS_PREFIX
							.length());
					if (query.endsWith("/")) {
						query = query.substring(0, query.length() - 1);
					}
		...
		browser.addProgressListener(new ProgressListener() {
			private void updateProgress(int loadingProgress) {
				Integer oldLoadingProgress = (Integer) browser
						.getData("CMJ_updateProgress.progress");
				if (!Utils.equals(oldLoadingProgress, loadingProgress)) {
					browser.setData("CMJ_updateProgress.progress",
							loadingProgress);
					new CMJ_updateLoadingProgress().asyncExec(browser,
							loadingProgress);
				}
			}
			...
			public void changed(ProgressEvent e) {
				...
				browser.setData("Browser.loading", true);
				updateProgress(e.current == e.total ? 100 : Math.min(e.current
						* 100 / e.total, 99));
			}
	...
	private static class NSCommandBrowserFunction extends BrowserFunction {
		...
		public Object function(Object[] args) {
			...
			new CMJ_commandReceived().asyncExec(getBrowser(), command,
					commandArgs);
			return null;
		}
	}
	...
	private static class NSConsolePrintingBrowserFunction extends
			BrowserFunction {
	...
	public NativeWebBrowser(JWebBrowser webBrowser, WebBrowserRuntime runtime) {
		...
		if (runtime == WebBrowserRuntime.DEFAULT) {
			String runtimeProperty = NSSystemPropertySWT.WEBBROWSER_RUNTIME
					.get();
	...
	public boolean navigate(String resourceLocation,
		...
		return Boolean.TRUE.equals(runSync(new CMN_navigate(),
				resourceLocation,
				parameters == null ? null : parameters.getPostData(),
				parameters == null ? null : parameters.getHeaders()));
	}
	...
	private static Pattern JAVASCRIPT_LINE_COMMENT_PATTERN = Pattern.compile(
			"^\\s*//.*$", Pattern.MULTILINE);
	...
	private static String fixJavascript(Browser browser, String script) {
		if ("mozilla".equals(browser.getBrowserType())) {
			...
			if (!isFixedJS) {
				// 2 workarounds for issues that seem to be happening with
				// XULRunner < 1.9.
				// Remove line comments, because it does not work properly on
				// Mozilla.
				// cf. bug: https://bugs.eclipse.org/bugs/show_bug.cgi?id=215335
				script = JAVASCRIPT_LINE_COMMENT_PATTERN.matcher(script)
						.replaceAll("");
				// encode the script, because it is passed as a URL in Mozilla
				// and gets URI-decoded.
	...
	public boolean executeJavascriptAndWait(String script) {
		return Boolean.TRUE
				.equals(runSync(new CMN_executeJavascript(), script));
	}
	...
	private static class CMN_executeJavascriptWithResult extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
			String script = (String) args[0];
	...
	private static class CMN_isBackNavigationEnabled extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
	...
	private static class CMN_isForwardNavigationEnabled extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
	...
	public boolean isForwardNavigationEnabled() {
		return Boolean.TRUE
				.equals(runSync(new CMN_isForwardNavigationEnabled()));
	}
	...
	private static void registerDefaultPopupMenu(final Browser browser) {
		...
		ResourceBundle bundle = ResourceBundle.getBundle(className.substring(0,
		...
		backMenuItem.setImage(new Image(browser.getDisplay(), JWebBrowser.class
				.getResourceAsStream(bundle.getString("SystemMenuBackIcon"))));
		...
		forwardMenuItem.setImage(new Image(browser.getDisplay(),
				JWebBrowser.class.getResourceAsStream(bundle
						.getString("SystemMenuForwardIcon"))));
		...
		reloadMenuItem.setImage(new Image(browser.getDisplay(),
				JWebBrowser.class.getResourceAsStream(bundle
						.getString("SystemMenuReloadIcon"))));
		...
		stopMenuItem.setImage(new Image(browser.getDisplay(), JWebBrowser.class
				.getResourceAsStream(bundle.getString("SystemMenuStopIcon"))));
		...
		menu.addMenuListener(new MenuAdapter() {
			...
			public void menuShown(MenuEvent e) {
				...
				stopMenuItem.setEnabled(Boolean.TRUE.equals(browser
						.getData("Browser.loading")));
			}
		});
		browser.setMenu(menu);
	}

	private static class CMN_setDefaultPopupMenuRegistered extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
			Browser browser = (Browser) getControl();
	...
	public void setDefaultPopupMenuRegistered(
			boolean isDefaultPopupMenuRegistered) {
		runAsync(new CMN_setDefaultPopupMenuRegistered(),
	...
	private static class CMJ_invokeFunction extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			if (nativeWebBrowser.nameToFunctionMap != null) {
				WebBrowserFunction function = nativeWebBrowser.nameToFunctionMap
						.get(args[0]);
				if (function != null) {
		...
		public Object run(Object[] args) {
			...
			BrowserFunction browserFunction = new BrowserFunction(browser,
				...
				public Object function(Object[] arguments) {
					return new CMJ_invokeFunction().syncExec(getBrowser(),
							getName(), arguments);
				}
			};
	...
	public void registerFunction(WebBrowserFunction function) {
		...
		if (nameToFunctionMap == null) {
			...
			WebBrowserFunction oldFunction = nameToFunctionMap
					.get(functionName);
	...
	private static class CMN_unregisterFunction extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			BrowserFunction browserFunction = (BrowserFunction) browser
					.getData(key);
	...
	public void unregisterFunction(WebBrowserFunction function) {
		...
		String functionName = function.getName();
		WebBrowserFunction currentFunction = nameToFunctionMap
				.get(functionName);
	...
	private static class CMJ_getCredentials extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			WebBrowserAuthenticationHandler authenticationHandler = nativeWebBrowser
					.getAuthenticationHandler();
			if (authenticationHandler == null) {
				return new Object[] { true, null, null };
			}
			String resourceLocation = (String) args[0];
			Credentials credentials = authenticationHandler.getCredentials(
					webBrowser, resourceLocation);
			if (credentials == null) {
				return new Object[] { false, null, null };
			}
			return new Object[] { true, credentials.getUserName(),
					credentials.getPassword() };
		}
	}

	private static class CMN_setAuthenticationHandler extends
			ControlCommandMessage {
		@Override
		public Object run(Object[] args) {
			...
			if (isActive) {
				AuthenticationListener authenticationListener = new AuthenticationListener() {
					public void authenticate(AuthenticationEvent e) {
						Object[] result = (Object[]) new CMJ_getCredentials()
								.syncExec(browser, e.location);
				...
				browser.setData("Browser.authenticationListener",
						authenticationListener);
				browser.addAuthenticationListener(authenticationListener);
			} else {
				browser.removeAuthenticationListener((AuthenticationListener) browser
						.getData("Browser.authenticationListener"));
	...
	public void setAuthenticationHandler(
			WebBrowserAuthenticationHandler authenticationHandler) {
	...
	public boolean unloadAndDispose() {
		if (isNativePeerInitialized()) {
			// We return "isAlive" (and not "isDisposed") because it is easier
	...
	private static class CMN_print extends ControlCommandMessage {
		...
		public Object run(Object[] args) {
			...
			if (Utils.IS_WINDOWS && "ie".equals(browser.getBrowserType())) {
				try {
					Class<?> ieClass = Class
							.forName("org.eclipse.swt.browser.IE");
					Field webBrowserField = Browser.class
							.getDeclaredField("webBrowser");
					...
					if (ieClass.isInstance(swtWebBrowser)) {
						...
						OleAutomation swtBrowserAutomation = (org.eclipse.swt.ole.win32.OleAutomation) autoField
								.get(swtWebBrowser);
						int[] rgdispid = swtBrowserAutomation
								.getIDsOfNames(new String[] { "ExecWB",
										"cmdID", "cmdexecopt" });
						Variant[] rgvarg = new Variant[] {
								new Variant(OLE.OLECMDID_PRINT),
								new Variant(
										isShowingDialog ? OLE.OLECMDEXECOPT_PROMPTUSER
												: OLE.OLECMDEXECOPT_DONTPROMPTUSER), };
						int[] rgdispidNamedArgs = new int[] { rgdispid[1],
								rgdispid[2], };
						/* Variant pVarResult = */swtBrowserAutomation.invoke(
